...
## Keywords
PDF, compression, browser-extension, chrome-extension, vue, typescript, wxt, pdf-lib, client-side, privacy, local-processing

## Annotation
PDF Compressor Extension - браузерное расширение для локального сжатия PDF файлов без отправки на сервер. Использует pdf.js для рендеринга страниц и pdf-lib для создания оптимизированного PDF. Обеспечивает конфиденциальность данных - все операции выполняются локально в браузере.
...
The compression workflow follows this sequence:
1. File validation via [`FileHandler`](src/services/fileHandler.ts)
2. PDF loading and page rendering via pdf.js
3. Canvas-based JPEG compression with configurable quality
4. New PDF creation via pdf-lib with optimized structure
5. Download trigger for the compressed file

### Key Dependencies
- **pdf-lib** (v1.17.1) — PDF creation and manipulation
- **pdfjs-dist** (v5.4.449) — PDF rendering and page extraction
- **Vue 3** (v3.5.27) — UI framework
- **WXT** (v0.20.14) — Web extension build tools
